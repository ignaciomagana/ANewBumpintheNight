...
def plot_corner(fig,plot_data,color,hist_alpha=0.7,bins=20,labelsize=14,title_size=14,logscale=False,vmax=None):
    ...
    * `label`: A latex string for axis labeling
    ...
    for i,key in enumerate(keys):
        ...
        bounds = getBounds(plot_data[key]['data'])
        med,upperError,lowerError = bounds
        ax.axvline(upperError+med,color=color,ls='--')
        ax.axvline(med-lowerError,color=color,ls='--')
        ax.set_title(r"${0:.2f}^{{+{1:.2f}}}_{{-{2:.2f}}}$".format(*bounds),fontsize=title_size)
